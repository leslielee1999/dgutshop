<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="身份" prop="roleId" sortable>
      ...
      <el-table-column align="center" label="编号" prop="code" sortable />
    ...
    <el-table-column align="center" label="今日体温" prop="temperature" sortable />
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button type="danger" size="mini" @click="handleDelete(scope.row)"
            >删除</el-button
          >
          <el-button type="info" size="mini" @click="showUpdateTemp(scope.row)"
            >上传今日体温</el-button
          >
        </template>
      </el-table-column>
    ...
    <!-- 增加对话框 -->
    <el-dialog :visible.sync="isShowForm" title="增加员工">
      <!-- <h3>增加管理员</h3> -->
      <el-form
        ref="admin"
        :rules="rules"
    ...
    <!-- 修改对话框 -->
    <el-dialog :visible.sync="isShowUpdate" title="修改信息">
      <!-- <h3>修改管理员信息</h3> -->
      <br />
      <el-form
        ref="admin"
      ...
      >
        ...
        <el-form-item label="员工状态" prop="status" v-if="admin.roleId == 2">
    ...
    <!-- 提交体温对话框 -->
    <el-dialog :visible.sync="isShowUpdateTemp" title="上传体温">
      <br />
      <el-form
        ref="admin"
        :rules="rules"
      ...
      >
        <el-form-item label="体温" prop="temperature">
          <el-input v-model="admin.temperature" />
        </el-form-item>
        <br />
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="info" @click="hiddnUpdateTemp()">取消</el-button>
        <el-button type="success" @click="handleUpdateTemp('admin')"
          >上传</el-button
        >
      </span>
    </el-dialog>
  ...
  methods: {
    ...
    handleCreate(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm("此操作将创建新员工, 是否继续?", "提示", {
    ...
    handleUpdate(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm("此操作将修改该员工信息, 是否继续?", "提示", {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            type: "warning",
            center: true,
    ...
    showUpdateTemp(row) {
      this.isShowUpdateTemp = true;
      this.admin = row;
      console.log(row);
    },
    hiddnUpdateTemp() {
      this.isShowUpdateTemp = false;
      this.getList();
    },
    handleUpdateTemp(formName) {
      console.log(this.admin);
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm("此操作将上传员工体温, 是否继续?", "提示", {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            type: "warning",
            center: true,
